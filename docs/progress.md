...
| 2026-09-25 | #45 | 下記の PR | GET /api/health（DB・エージェントの実行ファイルとバージョン・最後のバックアップ・直近の失敗）、packages/agent の detectAgent、jobs.latestFailure | 設定画面は Figma のフレームがないので判断待ち（issue にコメント） |
| 2026-09-26 | #45 | 下記の PR | Figma に「PC/設定」（21:2）を作り、/settings の画面（状態のリスト、選んだ項目の詳細、もう一度確かめる）とサイドバーの設定を実装。DESIGN.md 4.10、docs/design/README.md に追加 | E2E は /api/health の応答を差し替えて、エージェントが見つからない状態を確かめる |
