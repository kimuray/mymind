...
| フレーム名 | 画面 | URL（issue 011 の暫定決定） |
|---|---|---|
| `PC/今日` | 今日のタスク、3ペイン、状態の履歴 | `/` |
| `PC/朝の計画` | 持ち越しの判断、バックログからの追加、昨日のFBと調子 | `/morning` |
| `PC/振り返り` | Markdown の入力（書く／プレビュー）、今日のFB | `/reflection/:day?` |
| `PC/バックログ` | バックログの一覧、右ペインに日曜の棚卸し | `/backlog` |
| `PC/タイムライン` | 2週間の状態の区間、調子のレーン、期間の内訳 | `/timeline` |
| `PC/カレンダー` | 月のカレンダー、日ごとの記録、FBの後追い依頼 | `/calendar/:ym/:day?` |
